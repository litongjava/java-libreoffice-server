...
@Slf4j
public class OfficeOdpToPdfHandler {

  /**
   * 将上传的ODP文件转换为PDF并返回给客户端下载
   *
  ...
  public HttpResponse toPdf(HttpRequest request) {
    ...
    if (!extension.equals("odp")) {
      ...
      return Resps.json(request, RespBodyVo.fail("仅支持ODP文件"));
    }
    ...
    try {
      ...
      log.error("Error converting ODP file to PDF", e);
