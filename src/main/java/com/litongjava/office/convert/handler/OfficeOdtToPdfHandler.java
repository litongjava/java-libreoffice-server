...
@Slf4j
public class OfficeOdtToPdfHandler {

  /**
   * 将上传的ODT文件转换为PDF并返回给客户端下载
   *
  ...
  public HttpResponse toPdf(HttpRequest request) {
    ...
    if (!extension.equals("odt")) {
      ...
      return Resps.json(request, RespBodyVo.fail("仅支持ODT文件"));
    }
    ...
    try {
      ...
      log.error("Error converting ODT file to PDF", e);
