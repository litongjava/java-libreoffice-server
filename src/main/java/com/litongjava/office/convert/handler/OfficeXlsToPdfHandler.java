...
@Slf4j
public class OfficeXlsToPdfHandler {

  /**
   * 将上传的XLS/XLSX文件转换为PDF并返回给客户端下载
   *
   * @param request HttpRequest 对象，包含上传的文件
   * @return HttpResponse 对象，包含转换后的PDF文件
   */
  public HttpResponse toPdf(HttpRequest request) {
    ...
    if (!extension.equals("xls") && !extension.equals("xlsx")) {
      ...
      return Resps.json(request, RespBodyVo.fail("仅支持XLS/XLSX文件"));
    }
    ...
    try {
      ...
      log.error("Error converting XLS/XLSX file to PDF", e);
